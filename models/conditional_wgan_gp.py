...
class ConditionalDiscriminator(nn.Module):
    ...
    def forward(self, img, labels):
        ...
        # 拼接
        x = torch.cat([img_feat, class_embed], dim=1)

        # 判断
        return self.fc(x)


def gradient_penalty(discriminator, real_images, fake_images, labels, device="cuda"):
    """计算梯度惩罚"""
...
def test_models():
    ...
    # 创建模型
    generator = ConditionalGenerator(latent_dim=128, num_classes=16).to(device)
    discriminator = ConditionalDiscriminator(num_classes=16).to(device)

    # 测试输入
    batch_size = 4
    z = torch.randn(batch_size, 128).to(device)
    labels = torch.randint(0, 16, (batch_size,)).to(device)
    fake_images = torch.randn(batch_size, 4, 32, 32).to(device)
    ...
    # 测试梯度惩罚
    real_images = torch.randn(batch_size, 4, 32, 32).to(device)
    gp = gradient_penalty(discriminator, real_images, fake_images, labels, device)
    print(f"\n梯度惩罚: {gp.item():.4f}")
