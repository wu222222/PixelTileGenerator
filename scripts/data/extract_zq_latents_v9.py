"""
提取 VQ-VAE v9 z_q embeddings (连续向量)

功能:
- 加载 VQ-VAEv4 (v9 checkpoint)
- 提取所有 64x64 图片的 z_q (quantized embeddings)
- 保存为 z_q GAN 训练数据

输出:
- z_q.npy: [N, 128, 16, 16] 连续向量 (codebook lookup 后的结果)
- indices.npy: [N, 16, 16] 离散索引 (备用)
- names.json: 文件名列表
- config.json: 配置信息
"""
...
CONFIG = {
    ...
    "output_dir": "datasets/vqvae_v9_zq_data",
...
def main():
    print("=" * 60)
    print("提取 VQ-VAE v9 z_q Embeddings")
    print("=" * 60)
    ...
    with torch.no_grad():
        for i, img_path in enumerate(image_files):
            try:
                ...
                # indices: [1, 256] -> reshape to [1, 16, 16]
    ...
    all_zq = np.concatenate(all_zq, axis=0)
    all_indices = np.concatenate(all_indices, axis=0)
    print(f"\nz_q 形状: {all_zq.shape}")
    print(f"Indices 形状: {all_indices.shape}")
    ...
    # z_q 统计
    print(f"\nz_q 统计:")
    print(f"  范围: [{all_zq.min():.4f}, {all_zq.max():.4f}]")
    print(f"  均值: {all_zq.mean():.4f}")
    print(f"  标准差: {all_zq.std():.4f}")

    # 保存
    np.save(output_dir / "z_q.npy", all_zq)
    ...
    with open(output_dir / "config.json", "w") as f:
        json.dump({
            ...
            "num_samples": len(all_zq),
            "zq_shape": list(all_zq.shape[1:]),
