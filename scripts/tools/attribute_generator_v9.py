"""
Attribute-Controlled Texture Generator (v9 适配)

输入: 滑杆指定属性值 (structure, entropy, brightness, ...)
输出: 符合属性的 64x64 纹理

原理: 在属性空间中找到最近邻 z_q, 加权插值后解码
"""
...
CONFIG = {
    ...
    "attribute_data": "checkpoints/v9_attribute_space/attributes.json",
    "scaler_params": "checkpoints/v9_attribute_space/scaler_params.json",
...
def load_models():
    global vqvae, zq_vectors, names, attr_matrix, attr_keys
    ...
    print(f"加载 {len(names)} 个 z_q, shape: {zq_vectors.shape}")
...
def generate_from_attributes(target_attrs):
    ...
    流程: 属性 → KNN 找最近邻 → 直接用该样本的 z_q → decoder
    """
    global nn_model, scaler_mean, scaler_scale, attr_keys, zq_vectors, attr_ranges, vqvae
    ...
    z_selected = zq_vectors[nearest_idx]

    device = CONFIG["device"]
    z_tensor = torch.FloatTensor(z_selected).unsqueeze(0).to(device)

    # nearest codebook projection
    codebook = vqvae.vq.embedding
    B, C, H, W = z_tensor.shape
    z_flat = z_tensor.permute(0, 2, 3, 1).reshape(-1, C)
    cb = codebook.weight.data
    dist = torch.cdist(z_flat, cb)
    nearest = dist.argmin(dim=1)
    z_proj = cb[nearest].reshape(B, H, W, C).permute(0, 3, 1, 2)

    with torch.no_grad():
        img = vqvae.decode(z_proj)
...
<head>
    <meta charset="UTF-8">
    <title>Attribute Generator v9</title>
    <style>
...
<body>
    <h1>Attribute Generator v9</h1>
    <p class="subtitle">64x64 纹理属性控制生成</p>
    ...
    <script>
        ...
        function generate() {
            document.getElementById('preview-img').src = '/generate?' + getAttrUrl();
        ...
        function generateBatch() {
            ...
            for (let i = 0; i < 4; i++) {
                ...
                img.src = '/generate?' + getAttrUrl() + '&seed=' + Math.floor(Math.random() * 999999);
        ...
        function randomize() {
            attrKeys.forEach(k => {
                ...
                document.getElementById('attr-' + k).value = r.min + Math.random() * (r.max - r.min);
                updateAttr(k);
            });
            generate();
        }
...
def main():
    print("=" * 60)
    print("Attribute Generator v9 (64x64)")
    print("=" * 60)
    load_models()
