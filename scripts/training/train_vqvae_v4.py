...
CONFIG = {
    ...
    # 保存
    "checkpoint_dir": "checkpoints/vqvae_v4",
    "save_every": 50,
    "sample_every": 25,
...
def train():
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        if (epoch + 1) % CONFIG["save_every"] == 0:
            checkpoint_path = checkpoint_dir / f"vqvae_v4_epoch_{epoch+1:03d}.pth"
            torch.save({
                "epoch": epoch + 1,
        ...
        if avg_total_loss < best_loss:
            ...
            best_path = checkpoint_dir / "vqvae_v4_best.pth"
            torch.save({
                "epoch": epoch + 1,
                "model_state_dict": model.state_dict(),
                "optimizer_state_dict": optimizer.state_dict(),
    ...
    # 保存最终模型
    final_path = checkpoint_dir / "vqvae_v4_final.pth"
    torch.save({
        "epoch": CONFIG["epochs"],
        "model_state_dict": model.state_dict(),
        "optimizer_state_dict": optimizer.state_dict(),
