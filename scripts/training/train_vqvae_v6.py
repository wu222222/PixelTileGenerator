...
改进:
- Latent大小: 16×16 → 20×20 (256 → 400 tokens)
- 空间分辨率提升，边缘和纹理细节更好
"""
...
CONFIG = {
    ...
    # 保存
    "checkpoint_dir": "checkpoints/vqvae_v6",
    "save_every": 50,
    "sample_every": 25,
...
def train():
    ...
    tokens_per_sample = latent_size * latent_size

    print("=" * 60)
    print(f"VQ-VAE v6 训练 ({latent_size}×{latent_size} Latent)")
    print("=" * 60)
    print(f"设备: {CONFIG['device']}")
    print(f"Latent大小: {latent_size}×{latent_size} ({tokens_per_sample} tokens)")
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        if (epoch + 1) % CONFIG["save_every"] == 0:
            checkpoint_path = checkpoint_dir / f"vqvae_v6_epoch_{epoch+1:03d}.pth"
            torch.save({
                "epoch": epoch + 1,
        ...
        if avg_total_loss < best_loss:
            ...
            best_path = checkpoint_dir / "vqvae_v6_best.pth"
            torch.save({
                "epoch": epoch + 1,
                "model_state_dict": model.state_dict(),
                "optimizer_state_dict": optimizer.state_dict(),
    ...
    # 保存最终模型
    final_path = checkpoint_dir / "vqvae_v6_final.pth"
    torch.save({
        "epoch": CONFIG["epochs"],
        "model_state_dict": model.state_dict(),
        "optimizer_state_dict": optimizer.state_dict(),
