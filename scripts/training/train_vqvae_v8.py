...
实验目的: 64×64 瓦片适配
- 新架构 v3: 双阶段下/上采样 (64→32→16→32→64)
- latent_size=16 (256 tokens, 与v2一致)
- 码本 512 (比v7的256更大, 适配更高分辨率)
- batch_size=16 (64×64 像素量4倍于32×32, 需减小)
"""
...
from models.vq_vae_v3 import VQVAEv3
...
CONFIG = {
    ...
    "mse_weight": 0.5,
    "l1_weight": 0.5,
    "checkpoint_dir": "checkpoints/vqvae_v8",
    "save_every": 50,
    "sample_every": 25,
...
def save_samples(model, dataset, device, save_dir, epoch, num_samples=8):
    ...
    with torch.no_grad():
        for i, idx in enumerate(indices):
            ...
            comparison = Image.new("RGBA", (256 * 2 + 10, 256))
...
def train():
    print("=" * 60)
    print("VQ-VAE v8 训练 (64×64 适配)")
    print("=" * 60)
    print(f"设备: {CONFIG['device']}")
    print(f"架构: v3 双阶段下/上采样")
    print(f"输入尺寸: 64×64")
    print(f"隐藏通道: {CONFIG['hidden_channels']}")
    print(f"码本大小: {CONFIG['num_embeddings']}")
    print(f"Latent: {CONFIG['latent_size']}×{CONFIG['latent_size']} ({CONFIG['latent_size']**2} tokens)")
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        if (epoch + 1) % CONFIG["save_every"] == 0:
            checkpoint_path = checkpoint_dir / f"vqvae_v8_epoch_{epoch+1:03d}.pth"
            torch.save({
                "epoch": epoch + 1,
        ...
        if avg_total_loss < best_loss:
            ...
            best_path = checkpoint_dir / "vqvae_v8_best.pth"
            torch.save({
                "epoch": epoch + 1,
                "model_state_dict": model.state_dict(),
                "optimizer_state_dict": optimizer.state_dict(),
    ...
    final_path = checkpoint_dir / "vqvae_v8_final.pth"
    torch.save({
        "epoch": CONFIG["epochs"],
        "model_state_dict": model.state_dict(),
        "optimizer_state_dict": optimizer.state_dict(),
